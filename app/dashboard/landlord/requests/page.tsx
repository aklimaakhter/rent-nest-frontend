...
import { getLandlordRequests, updateRequestStatusAction } from "./_action/requestAction";
...
export default function LandlordRequestsPage() {
  ...
  const handleStatusUpdate = async (id: string, newStatus: "APPROVED" | "REJECTED") => {
    try {
      const res = await updateRequestStatusAction(id, newStatus);
      if (res && res.success !== false) {
      ...
      {requests.length === 0 ? (
        ...
        <div className="space-y-4">
          {requests.map((req: any) => {
            ...
            return (
              <div key={reqId} className="border p-5 rounded-xl shadow-sm bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h3 className="font-bold text-lg text-gray-900">{req.property?.title}</h3>
                  <p className="text-sm text-gray-600">
                    Tenant: <span className="font-semibold">{req.tenant?.name}</span> ({req.tenant?.email})
                  </p>
