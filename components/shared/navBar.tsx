    ...
    <header className="border-b bg-white sticky top-0 z-50 w-full shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
          ...
          {/* Landlord Dashboard */}
          {role === 'LANDLORD' && (
            <Link href="/dashboard/landlord" className="text-sm font-medium text-gray-700 hover:text-emerald-600">
             Landlord Dashboard
            </Link>
          )}
          ...
          {/* Tenant Dashboard */}
          {role === 'TENANT' && (
            <Link href="/dashboard/tenant" className="text-sm font-medium text-gray-700 hover:text-emerald-600">
              Tenant Dashboard
            </Link>
          )}
          ...
          {/* Admin Dashboard */}
          {role === 'ADMIN' && (
            <Link href="/dashboard/admin" className="text-sm font-medium text-gray-700 hover:text-emerald-600">
              Admin Dashboard
            </Link>
          )}
        </nav>
